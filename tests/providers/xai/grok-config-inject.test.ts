...
describe("Grok config injection", () => {
  ...
  beforeEach(() => {
    root = mkdtempSync(join(tmpdir(), "occx-grok-inject-"));
  ...
  test("replaces the managed region idempotently", () => {
    ...
    expect(content).toContain("[model.occx-new-model]");
    expect(content).toContain("[model.occx-newer-model]");
  });
  ...
  test("emits a shared model_providers block and per-model references (grok 0.2.109+)", () => {
    ...
    const providerBlock = block.slice(block.indexOf("[model_providers.openccx]"), block.indexOf("[model."));
    expect(providerBlock).toContain('base_url = "http://127.0.0.1:10190/v1"');
    expect(providerBlock).toContain('api_backend = "responses"');
    expect(providerBlock).toContain('api_key = "openccx-loopback"');
    expect(providerBlock).toContain('extra_headers = { "x-openccx-grok" = "1" }');
    const table = block.slice(block.indexOf("[model.occx-cursor-grok-4-5]"));
    expect(table).toContain('model = "cursor/grok-4.5"');
    expect(table).toContain('model_provider = "openccx"');
  ...
  test("reserves user-owned [model.*] aliases outside the fence", () => {
    const configPath = join(grokHome, "config.toml");
    const userContent = '[model.occx-mine]\nmodel = "user/model"\nbase_url = "https://example.test/v1"\n';
    writeFileSync(configPath, userContent, "utf8");
    ...
    // a duplicate [model.occx-mine] header would invalidate the whole TOML.
    expect(content.match(/\[model\.occx-mine\]/g) ?? []).toHaveLength(1);
    expect(content).toContain("[model.occx-mine-2]");
  ...
  test("recognizes quoted and whitespace-padded user model headers (TOML-equivalent forms)", () => {
    ...
    expect(content).toContain("[model.occx-quoted-2]");
    expect(content).toContain("[model.occx-spaced-2]");
    expect(content).toContain("[model.occx-single-2]");
    // Exactly one bare-form [model.occx-quoted] must NOT exist (only the user's quoted header).
    expect(content).not.toContain("[model.occx-quoted]");
  ...
  test("decodes TOML unicode escapes in user model headers", () => {
    const configPath = join(grokHome, "config.toml");
    // \U0000006F and \u006F are both "o" — these headers canonically define model.occx-esc*.
    const userContent = '[model."\\U0000006Fcx-esc"]\nmodel = "user/esc"\n[model."\\u006Fcx-esc4"]\nmodel = "user/esc4"\n';
    writeFileSync(configPath, userContent, "utf8");

    injectGrokConfig(10100, [{ id: "esc" }, { id: "esc4" }], { grokHome });
    const content = readFileSync(configPath, "utf8");
    expect(content).not.toContain("[model.occx-esc]");
    expect(content).not.toContain("[model.occx-esc4]");
    expect(content).toContain("[model.occx-esc-2]");
    expect(content).toContain("[model.occx-esc4-2]");
    expect(() => Bun.TOML.parse(content)).not.toThrow();
  });
  ...
  test("sanitizes aliases, suffixes collisions, and escapes TOML strings", () => {
    ...
    expect(block).toContain("[model.occx-anthropic-claude-opus-4-8]");
    expect(block).toContain("[model.occx-same-path]");
    expect(block).toContain("[model.occx-same-path-2]");
  ...
  describe("user table reservation across TOML header spellings", () => {
    ...
    // Every spelling below addresses the SAME table as a generated [model.occx-mine]. grok's TOML
    ...
    const collidingSpellings: Array<[label: string, header: string]> = [
      ["quoted first segment", '["model"."occx-mine"]'],
      ["single-quoted first segment", "['model'.occx-mine]"],
      ["mixed quoting with whitespace", `[ "model" . 'occx-mine' ]`],
      ["bare (baseline)", "[model.occx-mine]"],
      ["array of tables", "[[model.occx-mine]]"],
      ["sub-table", "[model.occx-mine.extra]"],
      ["trailing comment", '[model."occx-mine"] # mine'],
    ];
    ...
    for (const [label, header] of collidingSpellings) {
      test(`reserves a user alias written as ${label}`, () => {
        ...
        expect(generated).toContain("[model.occx-mine-2]");
        expect(generated).not.toContain("[model.occx-mine]\n");
      });
    }
    ...
    test("does not reserve aliases from unrelated tables", () => {
      ...
      writeFileSync(
        configPath(),
        '[models.occx-mine]\nx = 1\n\n[model_providers.occx-mine]\ny = 2\n\n[auth_provider.occx-mine]\nz = 3\n',
        "utf8",
      );
      ...
      const written = readFileSync(configPath(), "utf8");
      expect(written.slice(written.indexOf(BEGIN_MARKER))).toContain("[model.occx-mine]");
    });
    ...
    test("an unclosed header inside a multiline string does not swallow the next real header", () => {
      ...
      // scan reserve "a" and miss occx-mine entirely, so we emitted a duplicate [model.occx-mine]
      ...
      expect(generated.slice(generated.indexOf(BEGIN_MARKER))).toContain("[model.occx-mine-2]");
      expect(generated.match(/^\[model\.occx-mine\]$/gm)).toHaveLength(1);
    });
  ...
  describe("non-loopback binds refuse auto-registration", () => {
    ...
    for (const hostname of [undefined, "", "127.0.0.1", "localhost", "::1", "[::1]"]) {
      test(`still registers when bound to ${hostname === undefined ? "(unset)" : hostname || "(empty)"}`, () => {
        ...
        expect(result).toMatchObject({ ok: true, changed: true });
        expect(readFileSync(configPath(), "utf8")).toContain("[model.occx-gpt-5-6-sol]");
      });
    }
