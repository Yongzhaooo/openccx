...
function latestUserText(parsed: OccxParsedRequest): string {
...
function recentTurnHasToolActivity(parsed: OccxParsedRequest): boolean {
  ...
  let lastAssistant: OccxAssistantMessage | undefined;
...
export function analyzeTerminalTurn(parsed: OccxParsedRequest, events: readonly AdapterEvent[]): TerminalTurnAnalysis {
...
function assistantMessageFromEvents(events: readonly AdapterEvent[], timestamp: number): OccxAssistantMessage | undefined {
  let text = "";
  let thinking = "";
  ...
  const content: OccxAssistantContentPart[] = [];
...
export function buildContinuationRequest(parsed: OccxParsedRequest, events: readonly AdapterEvent[]): OccxParsedRequest {
...
export interface GuardedEventStreamOptions {
  ...
  continuation: (parsed: OccxParsedRequest) => AsyncIterable<AdapterEvent> | Promise<AsyncIterable<AdapterEvent>>;
  adapterName?: string;
...
function mergeUsage(first: OccxUsage | undefined, second: OccxUsage | undefined): OccxUsage | undefined {
...
export async function* guardTerminalEventStream(options: GuardedEventStreamOptions): AsyncGenerator<AdapterEvent> {
  ...
  let continuations = 0;
  let accumulatedUsage: OccxUsage | undefined;
