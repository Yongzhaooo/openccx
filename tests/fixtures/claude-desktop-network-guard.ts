...
const allowed = new Set<string>(JSON.parse(process.env.OCCX_TEST_ALLOWED_ORIGINS ?? "[]"));
const deniedFile = process.env.OCCX_TEST_DENIED_REQUESTS;
...
function permit(input: Parameters<typeof fetch>[0]): void {
  ...
  if (["127.0.0.1", "localhost", "[::1]"].includes(url.hostname)) {
    try {
      const record = JSON.parse(readFileSync(join(process.env.OPENCCX_HOME!, "runtime-port.json"), "utf8"));
  ...
  throw new Error("OCCX_TEST_EXTERNAL_REQUEST_BLOCKED");
}
