...
function parsed(stream = false): OccxParsedRequest {
  return {
...
function antigravityProvider(overrides: Partial<OccxProviderConfig> = {}): OccxProviderConfig {
  return provider({
...
describe("google provider hardening", () => {
  ...
  test("Antigravity rejects a blank OAuth token", async () => {
    ...
    await expect(adapter.buildRequest(parsed())).rejects.toThrow(
      "google-antigravity oauth token missing — run occx login google-antigravity",
    );
  });
  ...
  test("garbage stays debug-dropped while comment keepalives are excluded", async () => {
    ...
    try {
      ...
      const dropped = getDebugLogEntries().filter(entry => entry.line.includes("[occx:frame-drop] google"));
