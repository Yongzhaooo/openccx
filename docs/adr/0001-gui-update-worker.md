...
The dashboard needs buttons for `occx sync` and openccx self-update. `occx sync` is safe to run in
the proxy process because it refreshes Codex config/catalog state. `occx update` is different: npm
...
API endpoints that create an update job in `OPENCCX_HOME/update-job.json`. The proxy starts a
detached hidden CLI worker, and the worker performs the install command and optional restart.

For npm installs, the worker runs the Node launcher path (`node bin/occx.mjs update --tag <tag>`) so
...
For npm installs specifically, `node occx.mjs update` already stops the proxy and reinstalls /
...
npm installs skip the probe-first path entirely, because the launcher only prints `occx start`
...
- Update status survives a proxy restart because it is stored in the openccx config directory.
...
- A completed install can still finish with `status: "failed"` when the replacement proxy never
  ...
  `occx start` and the Bun `--allow-scripts` reinstall path.
