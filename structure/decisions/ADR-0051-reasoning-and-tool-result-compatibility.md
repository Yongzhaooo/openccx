...
- 기존 구현 및 제약 조건: Reasoning-input sanitation already handled raw content and `occxr1:` envelopes; the replay cache already supplied a bounded, thread-scoped physical-route identity, but no record connected that identity to native `encrypted_content` provenance.
