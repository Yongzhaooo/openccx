...
export interface ClaudeAgentStartupSyncDeps {
  ...
  readHubRoster?: (config: OccxConfig) => readonly string[] | undefined;
}

/**
 * The hub's roster for a connected client, from the CACHE only (#4236).
 *
 * Startup deliberately makes no network call for this. The roster is a convenience here — `occx
 ...
 */
function cachedHubRoster(config: OccxConfig): readonly string[] | undefined {
 ...
 * stale `occx-*.md` files. A small forwarding gate delays only the successful transition;
 ...
 * context-window map used by `occx claude`; failure keeps startup available and falls back to an
...
export async function syncClaudeAgentDefsAtProxyStartup(
  ...
  try {
    ...
    try {
      ...
      // Startup remains best-effort. The next management mutation or `occx claude` launch can
