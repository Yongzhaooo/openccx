...
/**
 * Does the openccx dummy marker belong in the system environment?
 *
 * Keyed on the SAME resolver `occx claude` uses, so an auto config with no Claude auth
 ...
 *
 * NOTE this is a SNAPSHOT: the file only changes when this runs (proxy start, `occx
 * ensure`, or a settings save). `occx claude` re-resolves live on every launch.
 */
...
export function systemEnvMarkerMode(config: OccxConfig, deps: SystemEnvDeps = {}): "proxy" | "subscription" {
...
export function writeShellEnvFile(
  ...
  const lines = [
    `# Generated by openccx — do not edit manually`,
  ...
  if (systemEnvMarkerMode(config, deps) === "proxy") {
    ...
    // `OPENCCX_API_AUTH_TOKEN` or the hardened service token file, so the same ladder the
  ...
  // Auto-context (devlog 260712 020): same contract as `occx claude` / launchctl.
...
const SHELL_HOOK_MARKER = "# openccx claude-env hook";
 ...
 * remove the hook. That is the intended failure direction — removing an Openccx-owned block
 * is reversible on the next foreground `occx start`, whereas leaving a hook pointing at an
