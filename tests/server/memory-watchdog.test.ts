...
describe("GET /api/system/memory", () => {
  ...
  test("serializes only an allowlisted Bun runtime provenance, omitting it otherwise (#848)", async () => {
    const inherited = process.env.OCCX_BUN_RUNTIME_SOURCE;
    ...
    try {
      for (const source of ["override", "bundled", "process"]) {
        process.env.OCCX_BUN_RUNTIME_SOURCE = source;
      ...
      process.env.OCCX_BUN_RUNTIME_PATH = "/usr/local/bin/definitely-not-this-bun";
      expect((await read()).bunRuntimeSource).toBeUndefined();
      delete process.env.OCCX_BUN_RUNTIME_PATH;
      ...
      process.env.OCCX_BUN_RUNTIME_SOURCE = "system";
      expect((await read()).bunRuntimeSource).toBeUndefined();
    } finally {
      if (inherited === undefined) delete process.env.OCCX_BUN_RUNTIME_SOURCE;
      else process.env.OCCX_BUN_RUNTIME_SOURCE = inherited;
      delete process.env.OCCX_BUN_RUNTIME_PATH;
    }
