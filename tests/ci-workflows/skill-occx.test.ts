...
import { renderManagementSurface } from "../../scripts/generate-occx-skill-surface";
 ...
 */
const SKILL_DIR = repoPath("skills", "occx");
...
describe("skills/occx structure", () => {
  ...
  test("front matter names the skill and carries trigger words", () => {
    ...
    expect(front).toContain("name: occx");
    expect(front).toContain("description:");
    // Without these it will not activate on the tasks it covers.
    for (const trigger of ["occx", "openccx", "account pool", "usage report", "management API"]) {
...
describe("the generated surface reference cannot drift", () => {
  test("the committed file matches regeneration exactly", () => {
    // If this fails, run: bun scripts/generate-occx-skill-surface.ts
  ...
  test("it carries the do-not-edit marker", () => {
    ...
    expect(read("references/01_management_surface.md")).toContain("GENERATED by scripts/generate-occx-skill-surface.ts");
  });
});

describe("documented commands exist", () => {
  /**
   * Every `occx <word>` that is presented AS A COMMAND, reduced to its top-level name.
   *
   * Only inline code spans and fenced blocks count. Scanning raw prose produced two false
   * positives that are worth remembering: "driving occx programmatically" (a sentence) and
   * "there is no `occx request-history` command" -- a line whose entire purpose is to say the
  ...
  function documentedCommands(): Set<string> {
    ...
    for (const text of files) {
      ...
      for (const span of spans) {
        for (const match of span.matchAll(/\boccx ([a-z][a-z0-9-]*)/g)) names.add(match[1]!);
      }
    }
    return names;
  }
  ...
  test("every occx <command> named anywhere in the skill is a real command", () => {
    ...
    // it and conclude the tool is broken. This is the assertion that caught `occx request-history`,
...
describe("the consent boundary is stated, not implied", () => {
  test("SKILL.md forbids starring and does not offer a flag for it", () => {
    ...
    expect(text).not.toMatch(/occx\s+\S*star\s+--yes/);
  });
...
describe("access-key recipes keep plaintext outside agent sessions", () => {
  ...
  const secretBearingAccessKeyCommand =
    /\b(?:occx|openccx)(?:\.(?:exe|mjs|cmd|ps1))?["']?\s+(?:access\s+keys?|api-key)\s+(?:create\b|rotate\b(?!\s+(?:--json\s+)?(?:commit|abort)(?=\s|$)))/gm;
  ...
  test("all key aliases reject creation/start and preserve non-secret commit/abort", () => {
    for (const binary of ["occx", "openccx"]) {
  ...
  test("wrappers, shell continuations and inline examples cannot hide literal commands", () => {
    for (const command of [
      "& occx access keys create rotated --json",
      "command occx access key create rotated",
      "env occx api-key rotate old-id",
      "& 'C:\\Tools\\openccx.exe' api-key rotate old-id",
      "node /opt/bin/occx.mjs access key create rotated",
      "occx.cmd access key create rotated",
      "& './opencodex.ps1' access keys rotate old-id",
      "occx access key \\\n  create rotated --json",
      "occx access key `\r\n  create rotated --json",
      "occx access key ^\n  rotate old-id",
      "occx access key rotate COMMIT",
    ]) {
      expect(secretBearingCommandsInCode("```bash\n" + command + "\n```"), command).toHaveLength(1);
    }
    expect(secretBearingCommandsInCode("Run `occx api-key create rotated --json` next.")).toHaveLength(1);
    expect(secretBearingCommandsInCode("Do not run `occx api-key create rotated --json`.")).toHaveLength(1);
    expect(secretBearingCommandsInCode("Creation under `occx access key` returns plaintext.")).toEqual([]);
  });
  ...
  test("explicit management POST recipes are detected without banning commit or abort", () => {
    ...
    for (const command of [
      ...
      "occx connect rotate --admin-token-stdin --json",
  ...
  test("the original unsafe recipe is detected and every shipped page is scanned", () => {
    const original = "```bash\noccx access key list --json\noccx access key create rotated --json\n"
      + "occx access key remove <old-id> --yes --json\noccx access key list --json\n```";
  ...
  test("guidance distinguishes configuration confirmation from revocation authority", () => {
    ...
    for (const command of ["occx access key rotate commit", "occx access key remove"]) {
