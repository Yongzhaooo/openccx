...
beforeEach(() => {
  ...
  testDir = mkdtempSync(join(tmpdir(), "occx-usage-debug-"));
...
describe("appendUsageDebug", () => {
  function sample(extra: Partial<{ requestId: string; ts: number }> = {}) {
    return {
      ts: extra.ts ?? 1,
      requestId: extra.requestId ?? "occx-debug-1",
      provider: "chatgpt",
      model: "gpt-5.5",
  ...
  test("appends one JSON line with the expected shape and 0o600 perms", () => {
    ...
    expect(parsed.requestId).toBe("occx-debug-1");
  ...
  test("rotates to the most recent USAGE_DEBUG_KEEP_LINES once USAGE_DEBUG_MAX_LINES is exceeded", () => {
    ...
    for (let i = 0; i < total; i++) {
      appendUsageDebug(sample({ requestId: `occx-${i}`, ts: i }));
    }
    const path = usageDebugPath();
    const lines = readFileSync(path, "utf-8").split(/\r?\n/).filter(Boolean);
    ...
    expect(last.requestId).toBe(`occx-${total - 1}`);
    expect(first.requestId).toBe(`occx-${total - USAGE_DEBUG_KEEP_LINES}`);
  });
