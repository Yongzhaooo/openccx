...
/**
 * D3 activation evidence: the fence stamps `x-openccx-grok` on every registered
...
test("the managed fence stamps the grok attribution header on every model", () => {
  ...
  expect(block).toContain('extra_headers = { "x-openccx-grok" = "1" }');
  // The header lives in the shared [model_providers.openccx] block, inherited by every
  ...
  expect(providerBlock).toContain("x-openccx-grok");
});

test("the fence survives a write and keeps the header line parseable", () => {
  const root = mkdtempSync(join(tmpdir(), "occx-grok-attr-"));
  ...
  try {
    ...
    expect(content).toContain('extra_headers = { "x-openccx-grok" = "1" }');
  } finally {
...
const config = { port: 0, defaultProvider: "mock", providers: {} } as unknown as OccxConfig;
...
test("a request carrying the header is tagged grok; one without is not", async () => {
  ...
  await handleChatCompletions(chatReq({ "x-openccx-grok": "1" }), config, tagged).catch(() => {});
