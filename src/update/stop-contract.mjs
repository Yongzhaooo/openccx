/**
 * The exit code `occx stop` uses to say "teardown succeeded, history cleanup did not".
 *
 * This is plain ESM rather than TypeScript because it has two consumers on opposite sides
 * of a process boundary: `src/update/index.ts` and the Node launcher `bin/occx.mjs`, which
 ...
 * history-only stop indistinguishable from a config conflict, and `bin/occx.mjs` mirrors
