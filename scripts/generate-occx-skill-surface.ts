/**
 * Generates `skills/occx/references/01_management_surface.md` from the capability table.
 *
 ...
 * the help text. `tests/ci-workflows/skill-occx.test.ts` asserts the committed file matches this output, so a
 * capability added without regenerating fails CI instead of silently shipping a stale skill.
 *
 * Usage:
 *   bun scripts/generate-occx-skill-surface.ts            # write
 *   bun scripts/generate-occx-skill-surface.ts --check     # exit 1 if stale
 */
import { writeFileSync, readFileSync, existsSync } from "node:fs";
import { join } from "node:path";
import { CAPABILITIES, HEAD_CAPABILITIES, capabilityInvocation } from "../src/cli/capabilities";

const TARGET = join(import.meta.dir, "..", "skills", "occx", "references", "01_management_surface.md");

export function renderManagementSurface(): string {
  const lines: string[] = [];
  lines.push("<!-- GENERATED by scripts/generate-occx-skill-surface.ts. Do not edit by hand. -->");
  lines.push("<!-- Regenerate: bun scripts/generate-occx-skill-surface.ts -->");
  lines.push("");
  lines.push("# The `occx` management surface");
  lines.push("");
  lines.push("Every capability the CLI declares, with the management routes it drives and whether it");
  lines.push("mutates state. This file is generated from the same table `occx capabilities --json`");
  ...
  lines.push("");
  lines.push("```bash");
  lines.push("occx capabilities --json                 # the whole table");
  lines.push("occx capabilities --mutating-only --json  # only state-changing verbs");
  lines.push("occx capabilities --route /api/logs      # which verbs drive one route");
  lines.push("```");
  lines.push("");
...
if (import.meta.main) {
  ...
  if (process.argv.includes("--check")) {
    ...
    if (current === rendered) {
      console.log("skills/occx/references/01_management_surface.md is current.");
      process.exit(0);
    }
    console.error("skills/occx/references/01_management_surface.md is STALE.");
    console.error("Regenerate: bun scripts/generate-occx-skill-surface.ts");
    process.exit(1);
  }
