...
"common.discard": "破棄",
  ...
  "common.retry": "再試行",
  "auth.adminTokenTitle": "Openccx 管理者トークン (OPENCCX_ADMIN_AUTH_TOKEN)",
  ...
  "auth.adminTokenHelp": "これは Openccx 管理 API の管理者トークンで、プロバイダーの API キーではありません。プロキシは初回起動時に ~/.opencodex/admin-api-token（または $OPENCCX_HOME/admin-api-token）へ書き込み、OPENCCX_ADMIN_AUTH_TOKEN を設定するとそちらが優先されます。",
  "auth.adminTokenDocsLink": "確認方法",
  "app.logoAria": "openccx ロゴ",
  ...
  // startup health
  "startup.title": "起動安全性",
  "startup.subtitle": "再起動後にローカルプロキシへの接続が再接続ループになる前に、Codex が openccx へ到達できるか確認します。",
  "startup.refresh": "更新",
  ...
  "startup.summary.protected": "再起動後も openccx を利用できます",
  ...
  "startup.riskDetailCustomLocal": "Codex はカスタムローカルゲートウェイを参照しています。openccx はその再起動ライフサイクルを管理・検証できません。",
  "startup.riskDetailWindowsShim": "Launcher shim は対応する CLI スクリプトのみを保護し、Windows の Codex Desktop と codex.exe の直接起動はこれを迂回できます。",
  "startup.safeDetail": "現在のルーティングと起動方式は整合しています。再起動後に occx start を手動実行する必要はありません。",
  ...
  "startup.shimHint": "対応する Codex スクリプトランチャーの起動時に occx ensure を実行します。",
  "startup.healthy": "正常",
  ...
  "startup.tray.error": "Windows トレイ操作に失敗しました。occx tray status で詳細を確認してください。",
  ...
  "startup.codexRuntime.clampHidden": "Openccx が Codex {version} を使用したため、一部の reasoning effort オプションが非表示になりました。",
  "startup.codexRuntime.clampHiddenWithEfforts": "Openccx が Codex {version} を使用したため、一部の reasoning effort オプションが非表示になりました（削除: {efforts}）。",
  "startup.codexRuntime.olderBinary": "Openccx は古い Codex バイナリ（{version}）を使用しています。より新しいインストールが利用可能です。",

  // dashboard
  "dash.subtitle": "ローカル openccx プロキシ、そのプロバイダー、Codex にルーティングされるモデルのライブ状態です。",
  ...
  "dash.codexAutoStart": "Codex と一緒に openccx を起動",
  "dash.codexAutoStartHint": "インストール済み launcher shim に occx ensure の実行を許可します。この設定だけでは再起動保護はインストールされません。起動安全性で実際の状態を確認してください。",
  ...
  "dash.shadowCallTooltip": "Codex App はスレッドタイトル生成、コミットメッセージ生成、スキルオーケストレーションをバックグラウンドで呼び出します。使われるモデルはクライアントのバージョンによって変わるため、openccx は {models} をまとめて傍受します。これをオンにすると、それらの呼び出しを選択したモデルにリダイレクトします。",
  ...
  "dash.maintenanceHint": "Codex のモデルカタログを更新するか、より新しい openccx リリースをインストールします。",
  ...
  "dash.projectConfigTitle": "プロジェクトの Codex 設定が Openccx をバイパスします",
  "dash.projectConfigHint": "これらのリポジローカル設定は Openccx プロキシを上書きします(例: OpenCode Go に直接ルーティング)。~/.codex/config.toml のルーティングがそのプロジェクトで適用されるように削除してください。",
  "dash.checkUpdate": "更新を確認",
  "dash.updateTitle": "openccx を更新",
  ...
  "dash.updateStatus.running": "openccx を更新しています。",
  ...
  // providers
  "prov.subtitle": "openccx が Codex にルーティングする上流プロバイダーを設定します。アカウントでログインするか、プロバイダーを追加、または生の設定を編集します。",
  "prov.add": "プロバイダーを追加",
  ...
  "oauthTos.anthropicBody": "Openccx のような第三者プロキシ経由で Claude サブスクリプションの OAuth トークンを直接再利用することは、Anthropic がサポートする統合ではなく、アクセス制限につながる可能性があります。Claude サブスクリプションを使用するサポートされた Agent SDK 統合は別物です。",
  "oauthTos.highBody": "Openccx は第三者 OAuth パス経由で {provider} に接続します。サポート外の利用はアクセス制限や停止につながる可能性があります。",
  "oauthTos.elevatedBody": "Openccx は非公式 OAuth パス経由で {provider} に接続します。可能な場合は公式クライアントを使用してください。異常または自動化されたトラフィックは悪用とみなされ、アクセスが制限または停止される可能性があります。",
  "oauthTos.saferPath": "より安全な選択肢: 代わりに Openccx で API キーを設定してください。",
  ...
  "models.subtitle": "Codex に表示するモデルを切り替えます — ネイティブ GPT パススルーとルーティングプロバイダー、プロバイダー別(ヘッダーをクリックで折りたたみ)。非表示モデルはカタログとピッカーから外れますが、正確な id での直接呼び出しは可能です。変更は次回の Codex ターンで適用 — openccx は Codex の 5 分間モデルキャッシュを無効化するので再起動は不要です。",
  ...
  // subagents
  "sub.subtitle": "Codex の {cmd} は最初の 5 モデル(優先度順)のみをオーバーライドとして通知します。ここで最大 5 つを選んでください — ネイティブ gpt またはルーティング — openccx がカタログ優先度を設定し、これらが先頭に来るようにします。他のモデルも正確な名前で呼び出し可能です; これは表示のみを制御します。",
  "sub.featured": "おすすめ",
  "sub.advanced": "詳細設定",
  ...
  "logs.tabLogs": "ログ",
  "logs.tabDebug": "デバッグ",
  "logs.subtitle": "ローカル openccx プロキシを経由した最近のリクエスト(新しい順)。",
 ...
 "quota.fiveHourLimit": "5 時間上限",
  ...
  "pws.copyDoctor": "occx doctor をコピー",
...
"codexSet.custom.moveUp": "{title} を上へ移動",
  ...
  "codexSet.custom.notOwned": "developer_instructions は openccx の外部で作成されたため、ここでは編集できません。レイヤーとして管理するにはインポートしてください。",
  ...
  "codexSet.custom.baseReplaced": "model_instructions_file が {path} に設定されているため、openccx の外部で基本プロンプトが置き換えられています。",
...
"codexSet.lint.size": "このレイヤーは 8 KB を超えています。保存はできますが、リクエストのたびにトークンを消費します。",
  ...
  "codexSet.preset.korean.provenance": "よく要望される項目をもとに openccx 向けに作成しました。独自の文言であり、複製ではありません。",
...
"codexSet.dialog.notRendered": "確認したターンでは、このレイヤーは何も送信していません。各セクションは内容が変わったときだけ再送されるため、一度の確認では欠けて見えることがあります。",
  ...
  "codexSet.base.externalBlocked": "model_instructions_file はすでに {path} を指しており、openccx が書いた値ではありません。自分で消してから選んでください。",
  "nav.api": "API",
  "nav.integrations": "連携",
  ...
  "integrations.subtitle": "クライアントを openccx に接続し、認証情報の管理とクライアント設定の復元を行います。",
  ...
  "integrations.codex.body": "Codex の接続はプロキシサービスが管理します。openccx を起動すると適用され、サービスを停止するとネイティブのルーティングに戻ります。",
  ...
  "integrations.dialog.desktop.changes": "{path} に openccx 管理のゲートウェイプロファイルがある場合、Desktop は先に認証情報のない標準プロファイルを選択し、その後で古いプロファイルとバックアップを削除します。",
  "integrations.dialog.desktop.breakage": "Claude Desktop は openccx 経由のモデルではなく、標準の Claude に戻ります。",
  "integrations.dialog.desktop.undo": "再度有効にすると、保存済みのモデル割り当てから openccx プロファイルを再生成します。",
  ...
  "integrations.native.error.desktopCleanupIncomplete": "Claude Desktop は標準モードを指していますが、古い openccx 認証情報ファイルが残っています: {paths}。",
  ...
  "integrations.detail.grokAbsent": "設定に openccx ブロックがありません",
  "integrations.dialog.codex.title": "Codex 連携を解除しますか？",
  "integrations.dialog.codex.changes": "openccx は {path} から自身のルーティングを削除し、生成したプロファイルを削除し、ネイティブのモデルカタログを復元し、再開可能なスレッドをネイティブ Codex 用に再タグ付けします。",
  ...
  "integrations.dialog.codex.sideEffect": "openccx が設定を注入した後にルートのルーティングモデルを選択していた場合、解除するとその選択も削除され、再有効化しても復元できません。モデルをもう一度選択してください。外部の model_provider が Codex を所有している場合、openccx は古いジャーナルだけを削除し、設定・カタログ・履歴は変更しません。",
  "integrations.dialog.codex.confirm": "解除",
  "integrations.dialog.grok.title": "Grok Build 連携を解除しますか？",
  "integrations.dialog.grok.changes": "{path} から、openccx が印を付けたブロックだけを削除します。ブロック外に直接書いた内容はそのまま残します。",
  "integrations.dialog.grok.breakage": "解除すると、Grok Build から openccx のモデルエイリアスが消えます。xAI アカウントで使用していたモデルはそのままです。",
  "integrations.dialog.grok.undo": "openccx が loopback アドレスで実行中なら、再び有効にしたとき、現在利用できるモデル一覧からブロックを新しく書き込みます。",
  "integrations.dialog.grok.confirm": "解除",
  "integrations.native.msg.nonLoopbackRemoved": "Grok Build を自動登録できるのは、openccx が loopback アドレスで実行中の場合だけです。loopback アドレスを指していた以前のブロックを削除しました。",
  "integrations.native.msg.nonLoopbackRemovedNoop": "Grok Build を自動登録できるのは、openccx が loopback アドレスで実行中の場合だけです。削除する以前のブロックはありませんでした。",
  "integrations.native.msg.nonLoopbackSuperseded": "Grok Build を自動登録できるのは、openccx が loopback アドレスで実行中の場合だけです。その間に別の場所から設定へ新しいブロックが書かれたため、現在ファイルにあるブロックはこのリクエストが作成したものではありません。",
  "integrations.native.error.orphanedMarker": "{path} に openccx の開始マーカーはありますが、終了マーカーがありません。ブロックの終端を特定できないため、ファイルは変更していません。",
  ...
  "integrations.onboarding": "適用時は、先にバックアップを保存してから openccx プロバイダーブロックを 1 つだけ書き込みます。無効化時はそのブロックだけを削除し、保存されたスナップショットから復元できます。",
  "integrations.empty.title": "インストール済みのクライアントが検出されませんでした",
  "integrations.empty.body": "対応クライアントをインストールしてから、ここに戻って openccx を適用してください。",
  ...
  "integrations.error.conflict": "openccx の書き込み後に設定が変更されました。何も削除していません。",
  ...
  "integrations.bulk.body": "openccx が所有するブロックだけを削除します。各クライアントについて、変更前のスナップショットを保存します。",
  ...
  "integrations.dialog.overwrite.changesUnowned": "{path} で openccx が必要とする位置を、openccx が書いたのではないブロックが占めています。適用すると openccx が書くブロックに置き換わります。",
  "integrations.dialog.overwrite.changesForeign": "{path} の openccx ブロックに加えた変更は破棄され、openccx が書くブロックに置き換わります。",
  ...
  "integrations.semantics.opencode": "ディスクから直接起動した場合にのみ適用されます。occx opencode の環境注入が優先されます。",
  ...
  "integrations.semantics.dsh": "Openccx が管理するのは $DSH_HOME/settings.yaml 内の llm-pi-ai.providers.openccx だけです。DSH はこのプロバイダーをホットリロードし、既定のモデルと deepseek-official は変更しません。現在はループバック専用で、実際の認証情報は書き込みません。",
  "integrations.semantics.mcode": "custom_provider.openccx のみを管理します。既定モデルと MiniMax ログインは変更しません。",
  "integrations.semantics.zcode": "~/.zcode/v2/config.json の provider.openccx のみを管理します。Z.ai ログインと他のプロバイダーは変更しません。変更後は ZCode を再起動してください。",
  "integrations.semantics.prime": "Prime Agent の models.json 内の providers.openccx のみを管理します。場所は ~/.prime/agent ですが、PRIME_AGENT_CODING_AGENT_DIR が設定されている場合はそちらが優先されます。他のプロバイダーとモデルオーバーライドは変更しません。新しいセッションから適用されます。",
  "integrations.semantics.aside": "このプロファイルの ~/.aside/u/<id>/models.json 内の providers.openccx のみを管理します。他のプロバイダーは変更しません。適用後は Aside を完全に終了してから開き直してください。",
  "integrations.semantics.raycast": "Raycast の providers.yaml に Openccx のプロバイダーエントリを追加し、ルーティングされたすべてのモデルを Raycast AI のモデル選択に表示します。Raycast Pro が必要です。",
  "integrations.semantics.omo": "omo の models.json にある providers.openccx のみを管理します。場所は ~/.omo/agent で、OMO_CODING_AGENT_DIR・SENPI_CODING_AGENT_DIR・PI_CODING_AGENT_DIR のいずれかが設定されている場合はそちらが優先されます。他のプロバイダーは変更しません。新しいセッションから適用されます。",
  "integrations.semantics.cline": "Cline CLI の providers.json と models.json の Openccx 項目を管理します。変更・同期前に Cline を終了し、完了後に再起動してください。元に戻すと両方の元ファイルが復元されます。既定のプロバイダーは変わりません。Cline で Openccx を選択してください。",
  ...
  "codexAuth.catalogRefreshPending": "変更は保存されましたが、Codex モデルカタログの更新が保留中です。occx sync を実行して再試行してください。",
  ...
  "accountPool.strategyDesc": "Openccx が新規/未紐付けタスクへアカウントを割り当てる方法です。",
  ...
  // api access page
  "api.title": "API アクセス",
  "api.subtitle": "生成した API キーで外部アプリから openccx プロキシに接続します。認証は {authHeader} ヘッダーで行い、エンドポイントごとに受け付けるヘッダーは下の表のとおりです。",
  ...
  "api.authLoopback": "ループバック (127.0.0.1 または ::1) は認証を省略します。リモートでは生成した occx_ キーまたは OPENCCX_API_AUTH_TOKEN が必要です。",
  ...
  "claude.injectAgentsDesc": "サブエージェントタブで選んだモデル(と現在のデフォルトモデル)をディスパッチ可能な Claude Code エージェント(occx-*)として登録します。次回セッションから適用されます。",
  ...
  "claude.smallFastModelNativeWarning": "未設定の場合、Openccx はヘルパーモデルの上書きを設定しません。Claude Code がネイティブの Sonnet モデルを使用し、ネイティブプロバイダーで料金が発生する可能性があります。",
  ...
  "grok.subtitle": "openccx が Grok 設定に登録したモデルです。",
  "grok.loading": "Grok の状態を読み込み中…",
  "grok.loadFail": "Grok 設定を読み取れませんでした。",
  "grok.notConfiguredTitle": "Grok Build が未設定です",
  "grok.notConfiguredHint": "Grok をインストールしてプロキシを再起動すると、openccx が管理ブロックを次の場所に書き込みます:",
  ...
  "cws.count.other": "その他",
  "cws.howTitle": "仕組み",
  "cws.howBody": "Codex に combo/<id> を要求します。Openccx はターゲットを選び、再試行可能な上流の失敗時のみホップします。利用可能なターゲットが残っていない場合、グローバルなデフォルトプロバイダーを使わずにフェイルクローズします。",
  ...
  "dash.codexClientCompactionHint": "既定ではオフで、認証済みループバックルーティング専用です。今後のコンパクションは、OpenCodeX と V2 プロバイダーのルーティングを維持したまま移植可能な平文要約を保存します。設定済みプロバイダーが要約を処理し、割り当てを消費する場合があります。既存の occx1 履歴は別途復旧が必要です。変更後は Codex を再起動してください。",
  ...
  "integrations.cursor.intro": "Cursor Private Inference はエージェントをローカルで実行し、loopback 経由で openccx と通信します。通常版の Cursor では利用できません。バックエンドがカスタムエンドポイントを呼び出すため、公開 HTTPS URL が必要です。このページから Cursor への書き込みは行いません。以下の値を自分で Cursor に貼り付けてください。",
  ...
  "integrations.cursor.apiKeyCredential": "openccx API キーのいずれか（このバインドには認証情報が必要です）",
  ...
  "integrations.cursor.modelsHint": "Cursor は独自のモデルテーブルから推論レベルの段階を決めるため、openccx が示せるのは予測のみです。コンテキスト欄にはデフォルトとオプトインのウィンドウ（Cursor の Max Mode）を表示します。",
  "integrations.cursor.ladderFromBundle": "推論レベルの段階は、インストール済みの Cursor Private Inference {version} バンドルから読み取りました。決めるのは Cursor で、openccx はその表を表示するだけです。",
