...
const DSH_VISIBLE_COPY: Record<(typeof LOCALES)[number], readonly [string, string, string]> = {
  en: [
    ...
    "DSH",
    "Openccx manages only llm-pi-ai.providers.openccx in $DSH_HOME/settings.yaml. DSH hot reloads this provider; your default model and deepseek-official stay unchanged. Currently loopback-only; no real credential is written.",
  ],
  fr: [
    "DeepSeek Harness (DSH)",
    "DSH",
    "Openccx gère uniquement llm-pi-ai.providers.openccx dans $DSH_HOME/settings.yaml. DSH recharge ce fournisseur à chaud ; votre modèle par défaut et deepseek-official restent inchangés. Seule l’adresse de bouclage est actuellement prise en charge ; aucun identifiant réel n’est écrit.",
  ],
  de: [
    "DeepSeek Harness (DSH)",
    "DSH",
    "Openccx verwaltet nur llm-pi-ai.providers.openccx in $DSH_HOME/settings.yaml. DSH lädt diesen Anbieter im laufenden Betrieb neu; Ihr Standardmodell und deepseek-official bleiben unverändert. Derzeit nur über Loopback; es werden keine echten Zugangsdaten geschrieben.",
  ],
  ja: [
    "DeepSeek Harness (DSH)",
    "DSH",
    "Openccx が管理するのは $DSH_HOME/settings.yaml 内の llm-pi-ai.providers.openccx だけです。DSH はこのプロバイダーをホットリロードし、既定のモデルと deepseek-official は変更しません。現在はループバック専用で、実際の認証情報は書き込みません。",
  ],
  ko: [
    "DeepSeek Harness (DSH)",
    "DSH",
    "Openccx는 $DSH_HOME/settings.yaml의 llm-pi-ai.providers.openccx만 관리합니다. DSH는 이 provider를 hot reload하며 기본 model과 deepseek-official은 변경하지 않습니다. 현재 loopback 전용이며 실제 credential을 기록하지 않습니다.",
  ],
  ru: [
    "DeepSeek Harness (DSH)",
    "DSH",
    "Openccx управляет только llm-pi-ai.providers.openccx в $DSH_HOME/settings.yaml. DSH применяет этот провайдер горячей перезагрузкой; модель по умолчанию и deepseek-official остаются без изменений. Сейчас поддерживается только loopback; реальные учётные данные не записываются.",
  ],
  tr: [
    "DeepSeek Harness (DSH)",
    "DSH",
    "Openccx yalnızca $DSH_HOME/settings.yaml içindeki llm-pi-ai.providers.openccx bölümünü yönetir. DSH bu sağlayıcıyı çalışırken yeniden yükler; varsayılan modeliniz ve deepseek-official değişmez. Şimdilik yalnızca geri döngü desteklenir; gerçek kimlik bilgisi yazılmaz.",
  ],
  zh: [
    "DeepSeek Harness (DSH)",
    "DSH",
    "Openccx 只管理 $DSH_HOME/settings.yaml 中的 llm-pi-ai.providers.openccx。DSH 会热重载该 provider；你的默认模型和 deepseek-official 保持不变。目前仅支持环回地址，且不会写入真实凭据。",
  ],
  "zh-TW": [
    "DeepSeek Harness (DSH)",
    "DSH",
    "Openccx 只管理 $DSH_HOME/settings.yaml 中的 llm-pi-ai.providers.openccx。DSH 會熱重載該 provider；你的預設模型與 deepseek-official 維持不變。目前僅支援 loopback，且不會寫入真實憑證。",
  ],
};
 ...
 * wrong in a different way if a translation drops it: which key Openccx
 * touches (`providers.openccx`, so the rest of the file is untouched), where
...
test("every locale keeps the three facts Aside's ownership sentence carries", async () => {
  for (const locale of LOCALES) {
    ...
    expect(semantics, `${locale} names the managed key`).toContain("providers.openccx");
