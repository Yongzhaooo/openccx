 ...
 * 10104 }`, only the `occx sync` writers honored the listener. Everything else — `occx claude`,
...
function hubConfig(listener: OccxConfig["unauthenticatedLoopbackListener"]): OccxConfig {
  return {
    port: HUB_PORT,
...
describe("companion hub: sync-managed and hardcoded clients agree", () => {
  test("the Codex target is the public port on loopback, with no admission header", () => {
    ...
    // A directly spawned app-server has no OPENCCX_API_AUTH_TOKEN to put in a header, so
  ...
  test("buildClaudeEnv lands on that exact origin without being taught about the listener", () => {
    // `occx claude` passes the live PUBLIC port and hardcodes 127.0.0.1. It is not changed by
  ...
  test("the ported form now agrees too: both writers land on the listener's port", () => {
    ...
    // the call sites were untouched). #4236's local-client unit closed the gap: `occx claude`
  ...
  test("with no listener EVERY local writer agrees on that address, Claude included", () => {
    // This is the case the first round of the local-clients fix got wrong. `occx sync` already
    ...
    const config = { ...hubConfig(undefined), apiKeys: [
      { id: "k1", name: "local", key: "occx_data_this_proxy_key", createdAt: "2026-01-01T00:00:00Z" },
    ] } as OccxConfig;
    ...
    const env = buildClaudeEnv({ ...config, claudeCode: { authMode: "proxy" } } as OccxConfig, HUB_PORT, {});
    ...
    expect(codexTarget.tokenEnv).toBe("OPENCCX_API_AUTH_TOKEN");
