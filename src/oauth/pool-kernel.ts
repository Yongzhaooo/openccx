import type { OccxAccountPoolRotationStrategy } from "../types";
...
const DEFAULT_STRATEGY: OccxAccountPoolRotationStrategy = "quota";
const VALID_STRATEGIES = new Set<OccxAccountPoolRotationStrategy>(["quota", "round-robin", "fill-first"]);
...
export function parseAccountPoolStrategy(raw: unknown): OccxAccountPoolRotationStrategy | null {
  if (typeof raw === "string" && VALID_STRATEGIES.has(raw as OccxAccountPoolRotationStrategy)) {
    return raw as OccxAccountPoolRotationStrategy;
  }
  return null;
}
...
export function normalizeAccountPoolStrategy(raw: unknown): OccxAccountPoolRotationStrategy {
