...
- 기존 구현 및 제약 조건: Compose persisted only the OCCX home, leaving Codex state on a read-only root; both products use incompatible auth.json formats.
...
- 다른 대안 대신 이 방식을 선택한 이유: It preserves existing paths, avoids credential-file collisions, and works when an older occx-state volume hides the image's seeded directory tree.
