...
export const RESERVE_METADATA_SOURCE_FIELD = "openccx_reserve_metadata_source";
/** Validated genuine source metadata on the existing catalog, never an authorization. */
export const RESERVE_SOURCE_CATALOG_FIELD = "openccx_reserve_source";
...
export function createReserveCatalogProjection(
  config: Pick<OccxConfig, "runtimeRole" | "hostname" | "unauthenticatedLoopbackListener" | "codexDesktopAuthless">,
  ...
  source.description = "Manual main-account Reserve through Openccx; recent upstream permission is required for every request.";
  // This qualified OCCX endpoint accepts the selector, not an OpenAI API-key model grant.
  ...
  delete source.openccx_account_observed_native;
  delete source.openccx_account_observed_selectors;
  return { source, mainSelectors: [...mainSelectors] };
}

/** Exact OCCX account projection, never another provider's similarly named model. */
export function isReserveCatalogProjection(entry: RawEntry): boolean {
  return entry.openccx_catalog_kind === CODEX_ACCOUNT_BOUND_CATALOG_KIND
