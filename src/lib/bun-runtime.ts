 ...
 *
 * openccx ships the Bun runtime via the `bun` npm dependency (esbuild-style:
 ...
 * launcher (bin/occx.mjs) and the durable service/shim integrations both need a
 ...
 */
export const BUN_RUNTIME_SOURCE_ENV = "OCCX_BUN_RUNTIME_SOURCE";
 ...
 */
export const BUN_RUNTIME_PATH_ENV = "OCCX_BUN_RUNTIME_PATH";
...
export function reportedBunRuntimeSource(
  ...
  // reported (a bare OCCX_BUN_RUNTIME_SOURCE inherited from an unrelated
 ...
 * be silently dropped on `occx ensure`, GUI start, restart, and update-relaunch, and the
 ...
 * shell nor its `OPENCCX_BUN_PATH`, and re-deriving would demote a correct `override`
...
export function durableBunRuntime(): DurableBunRuntime {
  ...
  // running executable. Re-reading OPENCCX_BUN_PATH here would let a project
 ...
 * manager-owned global package directory and survives across `occx update` — and fall back to the
