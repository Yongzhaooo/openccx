...
/** Byte-exact presets previously written by Openccx dashboard releases. */
export const LEGACY_OPENCCX_MODE_HINTS = [
...
/** Upgrade only known Openccx-owned values; user-authored variants stay byte-identical. */
export function canonicalizeOpenccxModeHint(text: string): string {
  return LEGACY_OPENCCX_MODE_HINTS.some(legacy => legacy === text)
