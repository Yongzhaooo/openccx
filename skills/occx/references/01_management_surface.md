<!-- GENERATED by scripts/generate-occx-skill-surface.ts. Do not edit by hand. -->
<!-- Regenerate: bun scripts/generate-occx-skill-surface.ts -->

# The `occx` management surface

Every capability the CLI declares, with the management routes it drives and whether it
mutates state. This file is generated from the same table `occx capabilities --json`
...
```bash
occx capabilities --json                 # the whole table
...
| `help` `--help` `-h` | Print the command list, or one command's usage with `occx help <command>`. |
...
- Start here when driving occx programmatically: it is the declared surface index, not a complete verb list.
...
### `occx inspect routing-analytics`
...
### `occx system codex-cli-update check`
...
Mint a single-use pairing code on a hub and print the exact `occx connect` line for one more machine.
...
### `occx account grok-reset-coupons`
