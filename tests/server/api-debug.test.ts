...
beforeEach(() => {
  ...
  isolatedCodexHome = installIsolatedCodexHome("occx-api-debug-codex-");
  testDir = mkdtempSync(join(tmpdir(), "occx-api-debug-"));
  ...
  delete process.env.OPENCCX_USAGE_DEBUG;
});
...
describe("management API /api/debug/logs", () => {
  ...
  test("caps limit query param at 2000", async () => {
    for (let i = 0; i < 5; i += 1) appendDebugLogLine(`[occx:test:${i}]`);
    const server = startServer(0);
    try {
...
describe("management API /api/debug/injection-logs", () => {
  test("returns buffered injection lines with seq cursor and limit", async () => {
    ...
    injectionDebugLog("[openccx] gpt-5.4: multi-agent guidance injected (surface=collab, 128 chars)");
    injectionDebugLog("[openccx] gpt-5.4: effort cap applied (ultra -> high, main turn)");
  ...
  test("caps limit query param at 2000", async () => {
    for (let i = 0; i < 5; i += 1) injectionDebugLog(`[openccx] inj:${i}`);
    const server = startServer(0);
    try {
...
describe("management API /api/debug/usage-logs", () => {
  test("tails usage-debug.jsonl from the running proxy home", async () => {
    ...
    try {
      ...
      expect(entries[0]!.seq).toBe(1);
      expect(entries[0]!.line).toContain("occx-usage-wire");
