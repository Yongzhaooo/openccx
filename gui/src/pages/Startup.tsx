...
const STARTUP_PAGE_CACHE_PREFIX = "occx.startup.page.v1:";
...
function deriveCodexRuntimeNotice(
  ...
  const doctorSync = shellChain(["occx doctor --fix-codex-runtime", "occx sync"], platform);
  if (clampActive) {
    return {
      warning: efforts
        ? t("startup.codexRuntime.clampHiddenWithEfforts", { version, efforts })
        : t("startup.codexRuntime.clampHidden", { version }),
      fix: newer ? doctorSync : "occx sync",
    };
  }
  if (newer) {
    return {
