 ...
 * three branches; openccx only implemented the third and hardcoded the macOS
...
describe("Claude Desktop configLibrary resolution", () => {
  ...
  test("OPENCCX_CLAUDE_DESKTOP_CONFIG_DIR wins over every branch and is used verbatim", () => {
    // Returned without join(): tests point this straight at a temp dir.
    const dir = resolveConfigLibraryDir({
      env: {
        OPENCCX_CLAUDE_DESKTOP_CONFIG_DIR: "/tmp/occx-override",
    ...
    expect(dir).toBe("/tmp/occx-override");
  });
...
/**
 * Desktop reads ONLY the profile named by `_meta.json`'s appliedId, so an openccx
...
describe("Claude Desktop status reports whether our profile is the active one", () => {
  async function statusWithMeta(meta: unknown): Promise<{ activeProfile: boolean | null }> {
    const dir = mkdtempSync(join(tmpdir(), "occx-desktop-active-"));
  ...
  test("appliedId pointing at another profile reports activeProfile false", async () => {
    const body = await statusWithMeta({
      ...
      entries: [
        ...
        { id: "22222222-2222-4222-8222-222222222222", name: "openccx" },
      ],
    });
  ...
  test("a readable appliedId with no openccx entry is a known false, not unknown", async () => {
  ...
  test("metadata without appliedId leaves activeProfile undeterminable", async () => {
    const body = await statusWithMeta({
      entries: [{ id: "22222222-2222-4222-8222-222222222222", name: "openccx" }],
    });
