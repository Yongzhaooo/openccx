 ...
 * still stripping proxy-minted `occxr1` envelopes no upstream can decrypt.
 */
...
import { OCCX_REASONING_PREFIX } from "../../src/responses/reasoning-envelope";
...
describe("sanitizeReasoningInputContent scoping", () => {
  ...
  test("preservation still strips an occxr1 envelope but keeps the plaintext content", () => {
    const item = reasoningItem({ encrypted_content: `${OCCX_REASONING_PREFIX}Zm9v` });
...
describe("DeepSeek Responses replay keeps reasoning on the wire", () => {
  function buildBody(provider: OccxProviderConfig): Record<string, unknown> {
