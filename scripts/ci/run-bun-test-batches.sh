...
# qualification lane sets OPENCCX_BUN_PATH so the batches actually execute on
...
readonly BUN_BIN="${OPENCCX_BUN_PATH:-bun}"
...
run_test_once() {
  ...
  log_file="$(mktemp -t occx-bun-test-batch.XXXXXX)"
