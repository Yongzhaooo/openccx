...
"common.discard": "丢弃",
  ...
  "common.loading": "加载中…",
  "common.retry": "重试",
  "auth.adminTokenTitle": "Openccx 管理员令牌 (OPENCCX_ADMIN_AUTH_TOKEN)",
  ...
  "auth.adminTokenHelp": "这是 Openccx 管理 API 的管理员令牌，不是服务商 API 密钥。代理首次启动时会写入 ~/.opencodex/admin-api-token（或 $OPENCCX_HOME/admin-api-token），设置 OPENCCX_ADMIN_AUTH_TOKEN 可覆盖该值。",
  ...
  // startup health
  "startup.title": "启动安全",
  "startup.subtitle": "检查重启后 Codex 是否仍能连接 openccx，避免本地代理路由陷入重复重连。",
  "startup.refresh": "刷新",
  ...
  "startup.shimHint": "支持的 Codex 脚本启动器运行时执行 occx ensure。",
  "startup.healthy": "正常",
  ...
  "startup.tray.error": "Windows 托盘操作失败。请运行 occx tray status 查看详情。",
  ...
  "startup.codexRuntime.clampHidden": "部分推理强度选项已隐藏，因为 Openccx 正在使用 Codex {version}。",
  "startup.codexRuntime.clampHiddenWithEfforts": "部分推理强度选项已隐藏，因为 Openccx 正在使用 Codex {version}（已移除：{efforts}）。",
  "startup.codexRuntime.olderBinary": "Openccx 正在使用较旧的 Codex 二进制文件（{version}）。检测到可用的较新安装。",

  // dashboard
  "dash.subtitle": "本地 openccx 代理、其提供方以及路由到 Codex 的模型的实时状态。",
  ...
  "dash.codexAutoStart": "随 Codex 启动 openccx",
  "dash.codexAutoStartHint": "允许已安装的 launcher shim 运行 occx ensure。此设置不会安装重启保护；请在启动安全中检查实际状态。",
  ...
  "dash.shadowCallTooltip": "Codex 应用会在后台调用辅助模型来生成线程标题、提交消息以及进行技能编排。该模型随客户端版本变化，因此 openccx 会同时拦截这些模型：{models}。启用此选项可将这些调用重定向到您选择的模型。",
  ...
  "dash.effortCapNone": "无上限",
    "dash.maintenance": "维护",
  "dash.maintenanceHint": "刷新 Codex 模型目录，或安装新的 openccx 版本。",
  ...
  "dash.projectConfigTitle": "项目 Codex 配置绕过了 Openccx",
  "dash.projectConfigHint": "这些仓库级设置会覆盖 Openccx 代理（例如直接走 OpenCode Go）。请移除它们，以便该项目使用 ~/.codex/config.toml 的代理路由。",
  ...
  // providers
  "prov.subtitle": "配置 openccx 路由到 Codex 的上游提供方。使用账户登录、添加提供方，或编辑原始配置。",
  "prov.add": "添加提供方",
  ...
  "oauthTos.anthropicBody": "通过 Openccx 等第三方代理直接复用 Claude 订阅 OAuth 令牌，并非 Anthropic 支持的集成方式，可能导致访问受限。可使用 Claude 订阅的受支持 Agent SDK 集成属于另一种方式。",
  "oauthTos.highBody": "Openccx 通过第三方 OAuth 路径连接 {provider}。如果该用法不受支持，访问可能会被限制或暂停。",
  "oauthTos.elevatedBody": "Openccx 通过非官方 OAuth 路径连接 {provider}。请尽量使用官方客户端；异常或自动化流量可能被视为滥用，访问可能会被限制或暂停。",
  "oauthTos.saferPath": "更安全的做法：改为在 Openccx 中配置 API 密钥。",
  ...
  "models.subtitle": "开关 Codex 可见的模型 — 原生 GPT passthrough 与已路由模型按提供方分组（点击标题可折叠）。隐藏的模型不会出现在目录和模型选择器中，但仍可按精确 id 直接调用。更改在下一个 Codex 回合生效 — openccx 会使 Codex 的 5 分钟模型缓存失效，因此无需重启。",
  ...
  // subagents
  "sub.subtitle": "Codex 的 {cmd} 仅将优先级最高的前 5 个模型作为覆盖项公开。在此最多选择 5 个 — 原生 gpt 或已路由模型 — openccx 会设置它们的目录优先级，使其正好排在前面。其他模型仍可按确切名称调用；此设置仅控制显示项。",
  "sub.featured": "精选",
  "sub.advanced": "高级",
  ...
  "logs.tabLogs": "日志",
  "logs.tabDebug": "调试",
  "logs.subtitle": "经过本地 openccx 代理的最近请求，最新在前。",
...
"codexSet.custom.moveUp": "上移 {title}",
  ...
  "codexSet.custom.notOwned": "developer_instructions 是在 openccx 外部写入的，因此无法在此编辑。请将其导入，以便作为层进行管理。",
  "codexSet.custom.adopt": "导入现有指令",
  "codexSet.custom.adoptConfirm": "导入为层",
  "codexSet.custom.adoptRefused": "无法导入现有值。",
  "codexSet.custom.baseReplaced": "model_instructions_file 已设置为 {path}，因此 openccx 外部的内容已替换基础提示词。",
...
"codexSet.lint.size": "此层超过 8 KB。仍可保存，但每次请求都会消耗令牌。",
  ...
  "codexSet.preset.korean.provenance": "根据常见的用户需求为 openccx 编写。文案由我们原创，并非复制。",
...
"codexSet.dialog.notRendered": "在我们读取的那一轮中，此层没有发送任何内容。各部分仅在内容变化时才会重新发送，因此单次采样可能看不到它。",
  ...
  "codexSet.base.externalBlocked": "model_instructions_file 已指向 {path}，且不是 openccx 写的。请先自行清除，再在此处选择。",
  "nav.openMenu": "打开菜单",
  "nav.closeMenu": "关闭菜单",
  "nav.goHome": "前往仪表板",
  "pws.refreshAllQuotas": "刷新全部额度",
  "pws.quotaRefreshDone": "额度检查完成",
  "integrations.subtitle": "将客户端连接到 openccx，管理凭据并恢复客户端配置。",
  ...
  "integrations.codex.body": "Codex 连接由代理服务管理。启动 openccx 时应用该连接；停止服务时恢复原生路由。",
  ...
  "integrations.dialog.desktop.changes": "如果 {path} 包含由 openccx 管理的网关配置，Desktop 会先选择新的无凭据标准配置，再移除旧配置及其备份。",
  "integrations.dialog.desktop.breakage": "Claude Desktop 将不再使用经由 openccx 路由的模型，而会恢复为标准 Claude。",
  "integrations.dialog.desktop.undo": "重新开启后，会根据已保存的模型分配重新生成 openccx 配置。",
  ...
  "integrations.native.error.desktopCleanupIncomplete": "Claude Desktop 已指向标准模式，但仍残留旧的 openccx 凭据文件：{paths}。",
  ...
  "integrations.detail.grokAbsent": "配置中没有 openccx 区块",
  "integrations.dialog.codex.title": "要停用 Codex 集成吗？",
  "integrations.dialog.codex.changes": "openccx 将从 {path} 中移除路由，删除其生成的配置，恢复原生模型目录，并将可恢复的线程重新标记为原生 Codex。",
  ...
  "integrations.dialog.codex.sideEffect": "如果你在 openccx 注入配置后选择了路由根模型，停用会移除该模型选择，再次启用也无法重建它；请重新选择模型。如果外部 model_provider 拥有 Codex，openccx 只会删除其过时日志，配置、目录和历史记录保持不变。",
  "integrations.dialog.codex.confirm": "停用",
  "integrations.dialog.grok.title": "要停用 Grok Build 集成吗？",
  "integrations.dialog.grok.changes": "只会从 {path} 中删除由 openccx 标记的区块。区块之外手动写入的内容将保持不变。",
  "integrations.dialog.grok.breakage": "停用后，Grok Build 中的 openccx 模型别名将消失。通过 xAI 账号使用的模型不受影响。",
  "integrations.dialog.grok.undo": "如果 openccx 正在 loopback 地址上运行，再次启用时会根据当前可用的模型列表重新写入区块。",
  "integrations.dialog.grok.confirm": "停用",
  "integrations.native.msg.nonLoopbackRemoved": "只有当 openccx 在 loopback 地址上运行时，才能自动注册 Grok Build。已删除之前指向 loopback 地址的区块。",
  "integrations.native.msg.nonLoopbackRemovedNoop": "只有当 openccx 在 loopback 地址上运行时，才能自动注册 Grok Build。没有需要删除的旧区块。",
  "integrations.native.msg.nonLoopbackSuperseded": "只有当 openccx 在 loopback 地址上运行时，才能自动注册 Grok Build。在此期间，其他进程向配置写入了新区块，因此文件中的当前区块并非由本次请求创建。",
  "integrations.native.error.orphanedMarker": "{path} 中有 openccx 开始标记，但没有结束标记。由于无法确定该区块的结束位置，因此未修改文件。",
  ...
  "integrations.onboarding": "应用时会先保存备份，再写入一个 openccx 提供方配置块。禁用时只移除该配置块，并可从保留的快照恢复。",
  "integrations.empty.title": "未检测到已安装的客户端",
  "integrations.empty.body": "安装受支持的客户端，然后返回此处应用 openccx。",
  ...
  "integrations.error.conflict": "openccx 写入后配置又发生了更改。未移除任何内容。",
  ...
  "integrations.bulk.body": "只会移除归 openccx 所有的配置块。每个客户端都会先保留一份写入前快照。",
  ...
  "integrations.dialog.overwrite.changesUnowned": "{path} 中 openccx 需要写入的位置被一个并非我们写入的代码块占用。应用会将其替换为 openccx 写入的代码块。",
  "integrations.dialog.overwrite.changesForeign": "{path} 中 openccx 代码块内你所做的修改会被丢弃，并替换为 openccx 写入的代码块。",
  ...
  "integrations.semantics.opencode": "仅适用于直接从磁盘启动；occx opencode 的环境注入优先。",
  ...
  "integrations.semantics.dsh": "Openccx 只管理 $DSH_HOME/settings.yaml 中的 llm-pi-ai.providers.openccx。DSH 会热重载该 provider；你的默认模型和 deepseek-official 保持不变。目前仅支持环回地址，且不会写入真实凭据。",
  "integrations.semantics.mcode": "仅管理 custom_provider.openccx，不会更改默认模型或 MiniMax 登录状态。",
  "integrations.semantics.zcode": "仅管理 ~/.zcode/v2/config.json 中的 provider.openccx，不会更改 Z.ai 登录状态或其他提供商。更改后请重启 ZCode。",
  "integrations.semantics.prime": "仅管理 Prime Agent 的 models.json 中的 providers.openccx；默认位于 ~/.prime/agent，若设置 PRIME_AGENT_CODING_AGENT_DIR 则以其为准。不会更改其他提供商或模型覆盖设置。对新会话生效。",
  "integrations.semantics.aside": "仅管理此配置文件的 ~/.aside/u/<id>/models.json 中的 providers.openccx。其他提供商保持不变。应用后请完全退出并重新打开 Aside。",
  "integrations.semantics.raycast": "在 Raycast 的 providers.yaml 中添加一个 Openccx 提供商条目，让所有已路由的模型出现在 Raycast AI 模型选择器中。需要 Raycast Pro。",
  "integrations.semantics.omo": "仅管理 omo 的 models.json 中的 providers.openccx，路径为 ~/.omo/agent，若设置了 OMO_CODING_AGENT_DIR、SENPI_CODING_AGENT_DIR 或 PI_CODING_AGENT_DIR 则以其为准。你的其他提供商保持不变。对新会话生效。",
  "integrations.semantics.cline": "管理 Cline CLI 的 providers.json 和 models.json 中的 Openccx 项目。修改或同步前请退出 Cline，完成后重新启动。撤销会恢复两个文件的原始内容。默认提供商保持不变，请在 Cline 中选择 Openccx。",
  ...
  "codexAuth.catalogRefreshPending": "更改已保存，但 Codex 模型目录仍待刷新。请运行 occx sync 重试。",
  ...
  "accountPool.strategyDesc": "Openccx 如何为新建/未绑定任务分配账号。",
  ...
  // api access page
  "api.title": "API 访问",
  "api.subtitle": "用生成的 API 密钥从外部应用访问 openccx 代理。认证使用 {authHeader} 请求头；各端点接受哪些请求头见下表。",
  ...
  "api.authTitle": "身份验证",
  "api.authLoopback": "回环绑定（127.0.0.1 或 ::1）会跳过身份验证。远程绑定需要生成的 occx_ 密钥或 OPENCCX_API_AUTH_TOKEN。",
  ...
  "claude.injectAgentsDesc": "将“子代理”页选中的模型（以及当前默认模型）注册为 Claude Code 可派遣的代理（occx-*）。从下一个会话开始生效。",
  ...
  "claude.smallFastModelNativeWarning": "留空时，Openccx 不会设置辅助模型覆盖项。Claude Code 可能使用其原生 Sonnet 模型，并可能产生原生提供方费用。",
 ...
 "modal.baseUrlPlaceholder": "https://...",
  ...
  "cws.count.other": "其他",
  "cws.howTitle": "工作原理",
  "cws.howBody": "在 Codex 中请求组合的公开模型名称；未设置时默认使用 combo/<id>。Openccx 仅在可重试的上游错误时切换目标。若没有可用目标，请求会直接失败，不会回退到全局默认提供方。",
  ...
  "grok.subtitle": "openccx 已注册到你的 Grok 配置中的模型。",
  "grok.loading": "正在加载 Grok 状态…",
  "grok.loadFail": "无法读取 Grok 配置。",
  "grok.notConfiguredTitle": "Grok Build 尚未接入",
  "grok.notConfiguredHint": "安装 Grok 后重启代理，openccx 会把托管块写入：",
  "grok.endpoint": "端点",
  "grok.colModel": "模型",
  ...
  "dash.codexClientCompactionHint": "默认关闭，仅适用于已认证的 loopback 路由。未来压缩会保存可移植的明文摘要，同时保留 OpenCodeX 与 V2 提供方路由；已配置的提供方可能处理摘要并消耗其额度。已有 occx1 历史仍需单独恢复。更改后请重启 Codex。",
  ...
  "integrations.cursor.intro": "Cursor Private Inference 会在本地运行其智能体，并通过环回地址与 openccx 通信。普通版 Cursor 无法如此工作：其后端会调用自定义端点，因此需要一个公开的 HTTPS URL。本页面不会向 Cursor 写入任何内容；请自行将以下值粘贴到 Cursor 中。",
  ...
  "integrations.cursor.apiKeyCredential": "你的任一 openccx API 密钥（此绑定需要凭据）",
  ...
  "integrations.cursor.modelsHint": "Cursor 会从自身的模型表中选择推理层级，因此 openccx 只能进行预测。“上下文”列会列出默认窗口和可选窗口（Cursor 的 Max Mode）。",
  "integrations.cursor.ladderFromBundle": "推理档位读取自已安装的 Cursor Private Inference {version} 包。档位由 Cursor 决定，openccx 只是展示它的表。",
