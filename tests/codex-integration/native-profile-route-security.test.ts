...
beforeEach(() => {
  testHome = mkdtempSync(join(tmpdir(), "occx-native-profile-route-security-"));
  process.env.HOME = testHome;
  process.env.OPENCCX_HOME = testHome;
  process.env.CODEX_HOME = join(testHome, "codex-home");
  process.env.OPENCCX_ADMIN_AUTH_TOKEN = "native-main-admin-token";
...
describe("native-main profile routes at the management admission boundary", () => {
  test("rejects missing or wrong credentials and hostile origins before every native operation dispatches", async () => {
    ...
    try {
      for (const operation of operations) {
        ...
        expect((await request({ "x-openccx-api-key": "wrong-admin-token" })).status).toBe(401);
        expect((await request({
          "x-openccx-api-key": "native-main-admin-token",
          origin: "https://attacker.example",
        })).status).toBe(403);
        expect(calls).toEqual([]);

        expect((await request({ "x-openccx-api-key": "native-main-admin-token" })).status).toBe(200);
      ...
      for (const operation of operations.filter(operation => operation.method === "POST")) {
        const request = (csrf?: string) => fetch(new URL(operation.path, server.url), {
          ...
          headers: {
            ...
            ...(csrf === undefined ? {} : { "x-openccx-csrf-token": csrf }),
          },
          body: operation.body ? JSON.stringify(operation.body) : undefined,
        });
  ...
  test("remote GUI sessions require the bound server, browser origin, and CSRF before native mutation dispatch", async () => {
    ...
    try {
      ...
      expect((await request({ ...base, "x-openccx-gui-origin": "https://evil.example.test" })).status).toBe(401);
      ...
      expect((await request(withoutHeader("x-openccx-gui-origin"))).status).toBe(401);
      expect((await request(withoutHeader("x-openccx-csrf-token"))).status).toBe(401);
      expect((await request({ ...base, Host: `127.0.0.1:${server.port}` })).status).toBe(401);
      expect((await request({ ...base, "x-openccx-csrf-token": "" })).status).toBe(401);
      expect(calls).toEqual([]);
