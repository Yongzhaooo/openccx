...
function developer(text: string): OccxMessage {
  return { role: "developer", content: text, timestamp: 0 };
}

function assistantWithCalls(calls: { id: string; name: string }[], text = ""): OccxMessage {
  return {
    role: "assistant",
    content: [
...
function toolResult(callId: string, name: string, output = "ok"): OccxMessage {
...
describe("openai-chat dangling tool_calls hardening", () => {
  ...
  test("T8 empty call id is minted so the call can be answered", () => {
    ...
    expect(minted).toMatch(/^call_occx_minted_\d+$/);
