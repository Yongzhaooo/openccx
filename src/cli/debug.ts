...
async function requireLiveProxy() {
  ...
  if (!live) {
    console.error("Proxy is not running. Start it with: occx start");
    process.exit(1);
  }
  return live;
}
...
function printScopeStatus(scope: DebugScope, view: DebugSettingsView): void {
  if (scope === "provider") {
    ...
    console.log("  Tail: occx debug provider logs [-f]");
  } else if (scope === "usage") {
    ...
    console.log("  Tail: occx debug usage logs [-f] (via running proxy API)");
    ...
    console.log("  View: occx observe claude-inbound");
  }
}

function envDebugEnabled(): boolean {
  return process.env.OCCX_DEBUG === "1"
...
async function printUsageLogs(follow: boolean): Promise<void> {
  ...
  try {
    ...
    const entries = await res.json() as { seq: number; line: string }[];
    for (const entry of entries) console.log(entry.line);
    if (entries.length === 0) console.log("(empty — enable with: occx debug usage on)");
    if (entries.length > 0) after = entries[entries.length - 1]!.seq;
  } catch (err) {
...
async function handleScopeCommand(scope: DebugScope, actionArgv: string[]): Promise<void> {
  ...
  if (action === "logs") {
    if (scope === "injection" || scope === "claude") {
      console.error(scope === "claude"
        ? "Use: occx observe claude-inbound"
        : "Injection debug has no buffered log stream; use: occx observe injection");
      process.exit(1);
    }
  ...
  console.error(scope === "injection" || scope === "claude"
    ? `Usage: occx debug ${scope} on|off|status|reset`
    : `Usage: occx debug ${scope} on|off|status|reset|logs [-f]`);
  process.exit(1);
}
...
function printTopLevelHelp(): void {
  ...
  console.log("");
  console.log("  occx debug provider on|off|status|reset|logs [-f]");
  console.log("  occx debug usage on|off|status|reset|logs [-f]");
  console.log("  occx debug injection on|off|status|reset");
  console.log("  occx debug claude on|off|status|reset");
  console.log("");
  console.log("Env defaults on start:");
  console.log("  provider → OCCX_DEBUG=1 (legacy OCCX_DEBUG_FRAMES still works)");
...
export async function handleDebugCommand(argv: string[]): Promise<void> {
  ...
  if (sub === "" || sub === "help" || sub === "--help" || sub === "-h") {
    ...
    if (!live) {
      ...
      console.log(`  provider → OCCX_DEBUG = ${envDebugEnabled() ? "on" : "off"}`);
