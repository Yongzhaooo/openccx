 ...
 * assigns it to process.env.OPENCCX_API_AUTH_TOKEN. Loads only when the env token
 * is empty and OCCX_API_TOKEN_FILE names a readable file.
 */
export function loadServiceTokenFromFile(env: Record<string, string | undefined>): string | null {
  if (env.OPENCCX_API_AUTH_TOKEN?.trim()) return null;
  const file = env.OCCX_API_TOKEN_FILE?.trim();
  if (!file) return null;
  try {
 ...
 * exec'ing the proxy, and WinSW native mode names it through `OCCX_API_TOKEN_FILE` — so under
 * a service the server has always seen `OPENCCX_API_AUTH_TOKEN` regardless of the calling
 * shell. A FOREGROUND `occx start` on the same machine had neither, so `assertServerAuthConfig`
 ...
 * this file as OPENCCX_API_AUTH_TOKEN, so doctor and start must inspect it even
