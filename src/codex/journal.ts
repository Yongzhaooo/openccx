 ...
 */
export const JOURNAL_PATH = join(CODEX_HOME, "openccx-journal.json");
 ...
 *
 * Only native (non-openccx-owned) config may be journaled, and native config
 * always supersedes an older snapshot. The first half stops a re-inject from
 * recording openccx's own routing as the user's original — which would survive
 * `occx stop` and make the injection unremovable. The second half is the #477 fix:
...
export function writeJournal(options: WriteJournalOptions = {}): void {
  ...
  // otherwise make openccx's own routing the user's permanent "original".
...
export function markJournalInjectedState(
  ...
  // The profile file is wholly generated by Openccx, so its latest exact hash remains safe
...
export function restoreJournalState(): RestoreJournalResult {
  ...
  // later bytes belong to Openccx. Keep the whole pair and its recovery evidence intact.
