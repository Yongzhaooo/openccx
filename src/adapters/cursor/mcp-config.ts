...
/**
 * One MCP server openccx starts/connects and exposes to the Cursor agent as callable tools.
 * Either `command` (stdio: openccx spawns the server as a child process) or `url`
 * (streamable-http: openccx connects to a remote MCP server) must be set.
 */
 ...
 */
export function resolveMcpServers(provider: OccxProviderConfig): ResolvedMcpServer[] {
