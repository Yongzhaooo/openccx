...
describe("claude context-window map (devlog 260712 B2)", () => {
  ...
  test("registers all four selector forms for routed models", () => {
    ...
    expect(map["claude-occx-cursor--gpt-5.6-luna"]).toBe(1_000_000);
  ...
  test("registers native slugs (bare + desktop alias + legacy alias)", () => {
    const map = buildClaudeContextWindows(["gpt-5.6-sol", "gpt-5.3-codex-spark", "gpt-5.4"], []);
    // Authoritative native overrides: gpt-5.6 natives follow Codex 272k, spark 100k.
    ...
    expect(map["claude-occx-native--gpt-5.6-sol"]).toBe(272_000);
    expect(map["gpt-5.3-codex-spark"]).toBe(100_000);
...
describe("auto-context (devlog 260712 020 + audit 021)", () => {
  ...
  test("auto-context marks a wide native slot, and turning it off unmarks anything under 1M", () => {
    ...
    const readable = effectiveModelEnv({ model: "claude-occx-native--gpt-5.6-sol" }, windows);
    expect(readable.ANTHROPIC_MODEL).toBe("claude-occx-native--gpt-5.6-sol");
