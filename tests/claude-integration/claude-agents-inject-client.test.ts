 ...
 * which is false on a connected client, so `~/.claude/agents/occx-*.md` stayed whatever a
 ...
 * invariant that keeps a user-authored `occx-*.md` from being overwritten.
 */
...
function tempDir(): string {
  const d = mkdtempSync(join(tmpdir(), "occx-agents-client-"));
  dirs.push(d);
  return d;
}
...
function clientConfig(extra?: Partial<OccxConfig>): OccxConfig {
  return {
    port: 10100,
...
describe("a hub-sourced roster drives the generated defs", () => {
  test("the hub's models become occx-*.md even when their provider is absent locally", () => {
    const dir = tempDir();
    ...
    expect(names).toContain("occx-grok-4-6");
    expect(names).toContain("occx-gpt-5-6-sol");
    ...
    const grok = defs.find(def => def.name === "occx-grok-4-6");
  ...
  test("the local roster is ignored entirely when a hub roster is supplied", () => {
    ...
    expect(defs.map(def => def.name)).toEqual(["occx-grok-4-6"]);
  });
  ...
  test("the picker's five-row cap still applies to a longer hub roster", () => {
    ...
    expect(defs.map(def => def.name)).not.toContain("occx-five-should-not-appear");
  });
});
...
describe("injectClaudeAgentDefs on a client", () => {
  test("writes the hub roster and keeps the ownership marker", () => {
    ...
    expect(written).toEqual(["occx-grok-4-6.md"]);
    const body = readFileSync(join(dir, "agents", "occx-grok-4-6.md"), "utf8");
    expect(body).toContain("generated-by: openccx");
  });

  test("a user-authored occx-* file without the marker is never overwritten or pruned", () => {
    const dir = tempDir();
    mkdirSync(join(dir, "agents"), { recursive: true });
    const mine = join(dir, "agents", "occx-grok-4-6.md");
    writeFileSync(mine, "---\nname: occx-grok-4-6\n---\nmy own agent\n");
    const stale = join(dir, "agents", "occx-handwritten.md");
    writeFileSync(stale, "---\nname: occx-handwritten\n---\nalso mine\n");
  ...
  test("injectAgents false still prunes owned files, roster override notwithstanding", () => {
    const dir = tempDir();
    injectClaudeAgentDefs(clientConfig(), {}, dir, ["xai/grok-4.6"]);
    expect(readdirSync(join(dir, "agents"))).toContain("occx-grok-4-6.md");
    ...
    expect(existsSync(join(dir, "agents", "occx-grok-4-6.md"))).toBe(false);
  });
});
...
describe("resolveHubRosterForClaude", () => {
  ...
  test("a live read hands back the hub's roster with no warning", async () => {
    ...
    const roster = await resolveHubRosterForClaude(CONNECTION, "occx_data_x", {
