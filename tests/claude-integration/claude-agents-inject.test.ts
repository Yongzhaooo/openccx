...
function tempDir(): string {
  const d = mkdtempSync(join(tmpdir(), "occx-agents-"));
  dirs.push(d);
  return d;
}
...
function cfg(extra?: Partial<OccxConfig>): OccxConfig {
  return { port: 10100, defaultProvider: "mock", providers: {}, ...extra } as OccxConfig;
}

function generatedBodies(config: OccxConfig, dir: string): string[] {
...
describe("buildClaudeAgentDefs (devlog 070 + audit 071)", () => {
  test("roster + pinned self mark only authoritative 1M windows; name collision suffix", () => {
    const windows = { "claude-occx-native--gpt-5.6-sol": 372_000, "claude-occx-cursor--gpt-5.6-sol": 1_000_000 };
    const dir = tempDir();
    writeFileSync(join(dir, "settings.json"), JSON.stringify({ model: "claude-occx-native--gpt-5.6-sol[1m]" }));
    ...
    expect(byName["occx-gpt-5-6-sol"]!.model).toBe("claude-occx-native--gpt-5.6-sol");
    expect(byName["occx-gpt-5-6-sol-2"]!.model).toBe("claude-occx-cursor--gpt-5.6-sol[1m]"); // collision suffix
    // Self pins the picker-saved default but cannot inherit an unsafe auto-context marker.
    expect(byName["occx-self"]!.model).toBe("claude-occx-native--gpt-5.6-sol");
  ...
  test("generated profiles retain catalog-derived 1M markers for Claude 4.6 and 4.7", async () => {
    ...
    expect(models).toEqual({
      "occx-claude-sonnet-4-6": "claude-sonnet-4-6[1m]",
      "occx-claude-opus-4-6": "claude-opus-4-6[1m]",
      "occx-claude-opus-4-7": "claude-opus-4-7[1m]",
    });
  });
  ...
  test("generated profiles preserve genuine routed [1m] ids and honor provider caps", async () => {
    ...
    const windows = buildClaudeContextWindows([], catalog);
    const dir = tempDir();
    writeFileSync(join(dir, "settings.json"), JSON.stringify({ model: "claude-occx-kimi--k3[1m]" }));
    const defs = buildClaudeAgentDefs(config, windows, dir);
    const models = Object.fromEntries(defs.map(def => [def.name, def.model]));

    expect(windows["claude-occx-kimi--k3"]).toBe(262_144);
    expect(windows["claude-occx-kimi--k3[1m]"]).toBe(1_048_576);
    expect(models).toEqual({
      "occx-k3-1m": "claude-occx-kimi--k3[1m]",
      "occx-self": "claude-occx-kimi--k3[1m]",
    });
    ...
    writeFileSync(join(cappedDir, "settings.json"), JSON.stringify({ model: "claude-occx-kimi--k3[1m]" }));
    const cappedDefs = buildClaudeAgentDefs(config, cappedWindows, cappedDir);

    expect(cappedWindows["claude-occx-kimi--k3[1m]"]).toBe(350_000);
    expect(Object.fromEntries(cappedDefs.map(def => [def.name, def.model]))).toEqual({
      "occx-k3-1m": "claude-occx-kimi--k3",
      "occx-self": "claude-occx-kimi--k3",
    });
  });

  test("marker case is honored and unknown windows keep the selector as-was", () => {
    const windows = { "claude-occx-cursor--gpt-5.6-sol": 1_000_000 };
    const dir = tempDir();
    // Uppercase [1M] spelling is a genuine marker (the CLI matches /\[1m\]/i).
    writeFileSync(join(dir, "settings.json"), JSON.stringify({ model: "claude-occx-cursor--gpt-5.6-sol[1M]" }));
    const defs = buildClaudeAgentDefs(cfg({ subagentModels: ["cursor/gpt-5.6-sol", "cursor/unknown-model"] }), windows, dir);
    const byName = Object.fromEntries(defs.map(d => [d.name, d]));
    expect(byName["occx-gpt-5-6-sol"]!.model).toBe("claude-occx-cursor--gpt-5.6-sol[1m]");
    // Incomplete metadata: no window entry -> selector preserved, never unmarked.
    expect(byName["occx-unknown-model"]!.model).toBe("claude-occx-cursor--unknown-model");
    expect(byName["occx-self"]!.model).toBe("claude-occx-cursor--gpt-5.6-sol[1M]");
  });

  test("placeholder guidance recommends haiku, never sonnet (issue #252)", () => {
    const dir = tempDir();
    writeFileSync(join(dir, "settings.json"), JSON.stringify({ model: "claude-occx-native--gpt-5.6-sol" }));
  ...
  test("unset roster seeds the defaults; explicit [] respected; no default model -> no self", () => {
    ...
    expect(explicit.map(d => d.name)).toEqual(["occx-self"]);
  ...
  test("rendered frontmatter quotes every scalar and parses back", () => {
    ...
    expect(body).toContain(`occx-route: ${def!.model}`);
  ...
  test("configured Claude Code subagent effort is rendered for roster and self definitions", () => {
    ...
    for (const effort of levels) {
      ...
      for (const def of defs) {
        ...
        expect(body).toContain(`<!-- occx-effort: ${effort} -->`);
      }
    }
  });
  ...
  test("regeneration removes a previously configured effort when the option is cleared", () => {
    ...
    const target = join(dir, "agents", "occx-gpt-5-6-sol.md");
  ...
  test("generated routed agents refuse the default blocked skill before its bundle expands", () => {
    ...
    expect(bodies).toHaveLength(2); // roster + occx-self
  ...
  test("generated blocked-skill guard mirrors custom names and honors explicit opt-out", () => {
    ...
    writeFileSync(join(customDir, "settings.json"), JSON.stringify({ model: "claude-occx-native--gpt-5.6-sol" }));
    ...
    writeFileSync(join(offDir, "settings.json"), JSON.stringify({ model: "claude-occx-native--gpt-5.6-sol" }));
...
describe("syncClaudeAgentDefs ownership contract (audit 071 #2/#3)", () => {
  ...
  test("writes, overwrites, and prunes ONLY marker-verified occx files", () => {
    const dir = tempDir();
    writeFileSync(join(dir, "settings.json"), JSON.stringify({ model: "claude-occx-native--gpt-5.6-sol" }));
    const defs = buildClaudeAgentDefs(cfg({ subagentModels: ["gpt-5.6-sol"] }), {}, dir);
    ...
    writeFileSync(join(agentsDir, "occx-custom.md"), "---\nname: occx-custom\n---\nuser file");
    writeFileSync(join(agentsDir, "occx-gpt-5-6-sol.md"), "user replaced this — no marker");
    const second = syncClaudeAgentDefs(buildClaudeAgentDefs(cfg({ subagentModels: [] }), {}, dir), dir)!;
    expect(second).toEqual(["occx-self.md"]);
    const remaining = readdirSync(agentsDir).sort();
    // occx-self rewritten; unowned occx-custom + user-replaced sol file both preserved.
    expect(remaining).toEqual(["occx-custom.md", "occx-gpt-5-6-sol.md", "occx-self.md"]);
    expect(readFileSync(join(agentsDir, "occx-gpt-5-6-sol.md"), "utf8")).toBe("user replaced this — no marker");
  });
  ...
  test.skipIf(!canSymlink)("symlinks are never followed or pruned", () => {
    ...
    try {
      symlinkSync(victim, join(agentsDir, "occx-linked.md"));
    } catch (err) {
    ...
    expect(readdirSync(agentsDir)).toContain("occx-linked.md");
  });
...
test("stale Desktop selectors retain the roster and configured blocked skills", () => {
  ...
  try {
    ...
    const stale = defs.find(def => def.name === "occx-self");
