...
"common.discard": "Discard",
  ...
  "auth.adminTokenHelp": "This is the Openccx management admin token, not a provider API key. The proxy writes it to ~/.opencodex/admin-api-token (or $OPENCCX_HOME/admin-api-token) on first start, and OPENCCX_ADMIN_AUTH_TOKEN overrides it.",
  ...
  "startup.subtitle": "Verify that Codex can reach openccx after a restart, before local proxy routing becomes a reconnect loop.",
  ...
  "startup.summary.protected": "openccx will be available after restart",
  ...
  "startup.riskDetailCustomLocal": "Codex points to a custom local gateway. openccx cannot manage or verify that gateway's restart lifecycle.",
  ...
  "startup.safeDetail": "The current routing and startup mechanism are consistent. No manual occx start should be required after restart.",
  ...
  "startup.shimHint": "Runs occx ensure when a supported Codex script launcher starts.",
  ...
  "startup.tray.error": "The Windows tray action failed. Check occx tray status for details.",
  ...
  "startup.codexRuntime.clampHidden": "Some reasoning effort options were hidden because Openccx used Codex {version}.",
  "startup.codexRuntime.clampHiddenWithEfforts": "Some reasoning effort options were hidden because Openccx used Codex {version} (removed: {efforts}).",
  "startup.codexRuntime.olderBinary": "Openccx is using an older Codex binary ({version}). A newer installation is available.",

  // dashboard
  "dash.subtitle": "Live status of the local openccx proxy, its providers, and the models routed into Codex.",
  ...
  "dash.codexAutoStart": "Start openccx with Codex",
  "dash.codexAutoStartHint": "Allows an installed launcher shim to run occx ensure. This setting does not install restart protection; check Startup safety for the effective state.",
  ...
  "dash.shadowCallTooltip": "Codex App makes background helper calls for thread title generation, commit message generation, and skill orchestration. The helper model changed across client versions, so openccx intercepts every model in this set: {models}. Enable this to redirect those calls to your chosen model.",
  ...
  "dash.maintenanceHint": "Refresh Codex's model catalog or install a newer openccx release.",
  ...
  "dash.projectConfigTitle": "Project Codex config bypasses Openccx",
  "dash.projectConfigHint": "These repo-local settings override the Openccx proxy (e.g. route to OpenCode Go directly). Remove them so ~/.codex/config.toml routing applies in that project.",
  "dash.checkUpdate": "Check update",
  "dash.updateTitle": "Update openccx",
  ...
  "dash.updateStatus.running": "Updating openccx.",
  ...
  // providers
  "prov.subtitle": "Configure the upstream providers openccx routes into Codex. Log in with an account, add a provider, or edit the raw config.",
  ...
  "oauthTos.anthropicBody": "Directly reusing Claude subscription OAuth tokens through a third-party proxy such as Openccx is not a supported Anthropic integration and may lead to access restrictions. Supported Agent SDK integrations that use Claude subscriptions are separate.",
  "oauthTos.highBody": "Openccx connects {provider} through a third-party OAuth path. Unsupported use may lead to access limits or suspension.",
  "oauthTos.elevatedBody": "Openccx connects {provider} through an unofficial OAuth path. Use the official client when possible; unusual or automated traffic may be treated as abuse and access may be limited or suspended.",
  "oauthTos.saferPath": "Safer option: configure an API key in Openccx instead.",
  ...
  "models.subtitle": "Toggle which models Codex sees — native GPT passthrough and routed providers, grouped by provider (click a header to collapse). Hidden models stay off the catalog + model picker but remain directly callable by exact id. Changes apply on the next Codex turn — openccx invalidates Codex's 5-min model cache so no restart is needed.",
  ...
  // subagents
  "sub.subtitle": "Codex's {cmd} advertises only the first 5 models (by priority) as overrides. Pick up to 5 here — native gpt or routed — and openccx sets their catalog priority so exactly these lead. Any other model is still callable by its exact name; this only controls what's shown.",
  ...
  "logs.tabLogs": "Logs",
  "logs.tabDebug": "Debug",
  "logs.subtitle": "Recent requests routed through the local openccx proxy, newest first.",
 ...
 "pws.noModelMatch": "No models match the filter.",
  ...
  "pws.copyDoctor": "Copy occx doctor",
...
"codexSet.custom.moveUp": "Move {title} up",
  ...
  "codexSet.custom.notOwned": "developer_instructions was written outside openccx, so it is not edited from here. Import it to manage it as a layer.",
  ...
  "codexSet.custom.baseReplaced": "model_instructions_file is set to {path}, so something outside openccx has replaced the base prompt.",
...
"codexSet.lint.size": "This layer is over 8 KB. It still saves, but it costs tokens on every request.",
  ...
  "codexSet.preset.korean.provenance": "Written for openccx from a user-requested staple. Our wording, not a copy.",
...
"codexSet.dialog.notRendered": "This layer sent nothing on the turn we read. Sections are only re-sent when they change, so an unchanged layer is absent from a single sample.",
  ...
  "codexSet.base.externalBlocked": "model_instructions_file already points at {path}, which openccx did not write. Clear it yourself before choosing an option here.",
  "nav.api": "API",
  ...
  "integrations.subtitle": "Connect clients to openccx, manage credentials, and restore client configuration.",
  ...
  "integrations.codex.body": "Codex wiring is owned by the proxy service. Starting openccx applies it; stopping the service restores native routing.",
  ...
  "integrations.detail.grokAbsent": "No openccx block in the config",
  ...
  "integrations.cursor.intro": "Cursor Private Inference runs its agent locally and talks to openccx on loopback. Regular Cursor cannot: its backend calls the custom endpoint and needs a public HTTPS URL. This page never writes to Cursor; paste the values below into Cursor yourself.",
  ...
  "integrations.cursor.apiKeyCredential": "One of your openccx API keys (this bind requires a credential)",
  ...
  "integrations.cursor.modelsHint": "Cursor picks the Reasoning ladder from its own model table, so openccx can only predict it. Context lists the default and the opt-in window (Cursor's Max Mode).",
  "integrations.cursor.ladderFromBundle": "Reasoning ladders read from the installed Cursor Private Inference {version} bundle. Cursor decides them; openccx only reports its table.",
  ...
  "integrations.dialog.codex.changes": "openccx will remove its routing from {path}, remove its generated profile, restore the native model catalog, and retag resumable threads for native Codex.",
  ...
  "integrations.dialog.codex.sideEffect": "If you selected a routed root model after openccx injected the config, disabling removes that model selection and turning the integration back on cannot reconstruct it; select the model again. If an external model_provider owns Codex, openccx removes only its stale journal and leaves the config, catalog, and history unchanged.",
  "integrations.dialog.codex.confirm": "Disable",
  "integrations.dialog.grok.title": "Disable the Grok Build integration?",
  "integrations.dialog.grok.changes": "Only the block marked by openccx will be removed from {path}. Content written outside the block will remain unchanged.",
  "integrations.dialog.grok.breakage": "Disabling removes the openccx model aliases from Grok Build. Models used with your xAI account remain available.",
  "integrations.dialog.grok.undo": "If openccx is running on a loopback address, turning this back on writes a new block from the models currently available.",
  "integrations.dialog.grok.confirm": "Disable",
  "integrations.dialog.desktop.title": "Disable Claude Desktop integration?",
  "integrations.dialog.desktop.changes": "If {path} contains an openccx-managed gateway profile, Desktop will first select a new credential-free standard profile, then remove the old profile and backup.",
  "integrations.dialog.desktop.breakage": "Claude Desktop will return to standard Claude instead of models routed through openccx.",
  "integrations.dialog.desktop.undo": "Turning this back on regenerates the openccx profile from your saved model assignments.",
  ...
  "integrations.native.msg.nonLoopbackRemoved": "Grok Build can be registered automatically only while openccx runs on a loopback address. The previous block that pointed to loopback was removed.",
  "integrations.native.msg.nonLoopbackRemovedNoop": "Grok Build can be registered automatically only while openccx runs on a loopback address. There was no previous block to remove.",
  "integrations.native.msg.nonLoopbackSuperseded": "Grok Build can be registered automatically only while openccx runs on a loopback address. Another process wrote a new block in the meantime, so the block now in the file was not created by this request.",
  "integrations.native.error.orphanedMarker": "{path} has an openccx start marker but no end marker. The file was left unchanged because openccx cannot determine where its block ends.",
  ...
  "integrations.native.error.desktopCleanupIncomplete": "Claude Desktop is pointed at standard mode, but old openccx credential files remain at: {paths}.",
  ...
  "integrations.onboarding": "Applying writes one openccx provider block after saving a backup. Disable removes only that block, and a retained snapshot can be restored.",
  "integrations.empty.title": "No installed clients were detected",
  "integrations.empty.body": "Install a supported client, then return here to apply openccx.",
  ...
  "integrations.error.conflict": "The config changed after openccx wrote it. Nothing was removed.",
  ...
  "integrations.bulk.body": "Only the openccx-owned block is removed. A pre-write snapshot is kept for each client.",
  ...
  "integrations.dialog.overwrite.changesUnowned": "A block we did not write occupies the settings openccx needs in {path}. Applying replaces it with the block openccx would write.",
  "integrations.dialog.overwrite.changesForeign": "Your edit inside the openccx block in {path} will be discarded and replaced with the block openccx would write.",
  ...
  "integrations.semantics.opencode": "Direct disk launches only; occx opencode environment injection takes precedence.",
  ...
  "integrations.semantics.dsh": "Openccx manages only llm-pi-ai.providers.openccx in $DSH_HOME/settings.yaml. DSH hot reloads this provider; your default model and deepseek-official stay unchanged. Currently loopback-only; no real credential is written.",
  "integrations.semantics.mcode": "Manages only custom_provider.openccx. Your default model and MiniMax login stay unchanged.",
  "integrations.semantics.zcode": "Manages only provider.openccx in ~/.zcode/v2/config.json. Your Z.ai login and other providers stay unchanged. Restart ZCode after changes.",
  "integrations.semantics.prime": "Manages only providers.openccx in Prime Agent's models.json — ~/.prime/agent unless PRIME_AGENT_CODING_AGENT_DIR redirects it. Your other providers and model overrides stay unchanged. Applies to new sessions.",
  "integrations.semantics.aside": "Manages only providers.openccx in this profile’s ~/.aside/u/<id>/models.json. Your other providers stay unchanged. Fully quit and reopen Aside after applying.",
  "integrations.semantics.raycast": "Adds an Openccx provider entry to Raycast's providers.yaml so every routed model appears in the Raycast AI model picker. Raycast Pro required.",
  "integrations.semantics.omo": "Manages only providers.openccx in omo's models.json — ~/.omo/agent unless OMO_CODING_AGENT_DIR, SENPI_CODING_AGENT_DIR or PI_CODING_AGENT_DIR redirects it. Your other providers stay unchanged. Applies to new sessions.",
  "integrations.semantics.cline": "Manages Openccx in Cline CLI providers.json and models.json. Stop Cline before changing or syncing these files, then restart. Undo restores both originals. Your default provider stays unchanged; select Openccx in Cline.",
  ...
  "codexAuth.catalogRefreshPending": "The change was saved, but the Codex model catalog refresh is pending. Run occx sync to retry.",
  ...
  "accountPool.strategyDesc": "How Openccx assigns an account to a new/unbound task.",
  ...
  "api.subtitle": "Use generated API keys to access the openccx proxy from external apps. Keys authenticate via the {authHeader} header; see the table below for what each endpoint accepts.",
  ...
  "api.authLoopback": "Loopback binds (127.0.0.1 or ::1) bypass authentication. Remote binds require a generated occx_ key or OPENCCX_API_AUTH_TOKEN.",
  ...
  "claude.injectAgentsDesc": "Registers the models picked on the Subagents tab (plus the current default model) as dispatchable Claude Code agents (occx-*). Applies from the next session.",
  ...
  "claude.smallFastModelNativeWarning": "When unset, Openccx leaves the helper-model overrides unset. Claude Code may use its native Sonnet model, which may incur charges from your native provider.",
  ...
  "cws.howBody": "Ask Codex for the combo's public model name. Without one, the default is combo/<id>. Openccx selects a target and hops only on retryable upstream failures. If no target remains available, the request fails closed instead of using the global default provider.",
  ...
  "grok.subtitle": "Models openccx has registered in your Grok config.",
  ...
  "grok.notConfiguredHint": "Start or restart the proxy with Grok installed and openccx writes a managed block into:",
